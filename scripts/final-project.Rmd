...
chips_rur_1988_filt %>%
  ...
  mutate(., diff_from_country_mean = base::mean(chips_rur_1988_filt$HNET88)-mean)


chips_rur_1995 %>%
  group_by(A1) %>%
  summarise(mean=base::mean(B602)) %>%
  mutate(., diff_from_country_mean = base::mean(chips_rur_1995$B602)-mean)

chips_rur_2007hhiexp %>%
  group_by(name_id) %>%
  summarise(mean=base::mean(income_net)) %>%
  mutate(., diff_from_country_mean = base::mean(chips_rur_2007hhiexp$income_net)-mean)

chips_rur_2013 %>%
  group_by(coun) %>%
  summarise(mean=base::mean(f01_1)) %>%
  mutate(., diff_from_country_mean = base::mean(chips_rur_2013$f01_1, na.rm=TRUE)-mean)
